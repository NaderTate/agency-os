...
> The agency's identity, mission, ICP, and brand voice live in **`context/business.md`**. The strategy, targets, and KPIs live in **`context/goals.md`**. Edit those two files to make this OS yours, every command downstream reads from them.

---

## What this is

It runs both sides of the agency: **sales** (a CRM + pipeline in `data/clients/`, priced from `context/services.md`) and **delivery** (a team in `data/team/` staffed onto active clients, with cost and margin). The work happens through these verbs:
...
| `/brief` | Morning brief: pipeline movement, AR, what to chase today, anchored to `context/goals.md`. One screen. |
...
| `/clickup <client>` | Push the client's delivery tasks (from `context/playbooks/`, plus extra items derived from any linked `recording:` meeting transcript) into a new ClickUp list named after them. |
...
- "import my clients from data/clients.csv" / "pull my list out of HubSpot" → run `/import`.
...
- "add a voice engineer named Maya at $65 an hour, 30 hours a week" → create `data/team/maya-chen.md` from `data/team/_template.md` (a hire; no dedicated command, just do it).
...
1. Read `context/business.md` (identity, mission, ICP, positioning, brand voice).
2. Read `context/goals.md` (north star, this-quarter targets, KPIs).
3. Read `context/services.md` (the catalog + pricing, needed for proposals and `/status` math).
4. Glob `data/clients/*.md` (skip `_template.md`) and read each one's `meta` block (including any `team:` assignments).
5. Glob `data/team/*.md` (skip `_template.md`) and read each member's `meta` block.
6. If `data/finance/invoices.md` exists, read the ledger (so AR is in context).
7. Return a one-screen snapshot: clients per stage, open pipeline value, signed MRR **vs the north star** from `context/goals.md`, net margin, AR outstanding + overdue, free team capacity, and the single most urgent next action (overdue invoices outrank everything). Under 130 words.
...
Every client is one file at `data/clients/<slug>.md`. The top is a fenced ` ```meta ` block that `/status` parses; the rest is freeform research + timeline. See `data/clients/_template.md`.
...
Every team member is one file at `data/team/<slug>.md` with a `meta` block: `name, slug, role, status (active/bench/inactive), rate (agency cost, USD/hr), capacity (billable hrs/week), skills`. See `data/team/_template.md`.
...
Invoices live in `data/finance/invoices.md`, a single markdown-table ledger. Each row has `id, client, amount, for, issued, due, status, paid_on`.
...
- **Price from `context/services.md`.** Never invent pricing. If a deal needs a custom number, base it on the catalog and say so.
...
- **Price and cost from the files.** Pricing comes from `context/services.md`; team cost comes from each member's `rate` in `data/team/`. Never invent either.
- **Demo-safe by default.** Both `data/clients/` and `data/team/` ship empty. Two CSV exports (`data/clients.csv`, `data/team.csv`) sit under `data/` to `/import`. Rich per-entity content lives in `data/onboarding/{clients,team}/<slug>.md` sidecars that `/import` auto-merges when present. The demo email is `demo@example.com`. Real outreach (`/kickoff`, `/remind`) only fires against real contact details you add yourself.
...
3. Make it yours: edit `context/business.md` (identity, ICP, voice), `context/goals.md` (north star, quarterly targets, KPIs), and `context/services.md` (offerings + pricing). The CRM and team roster both ship empty, run `/import data/clients.csv` and `/import data/team.csv` to fill them, then keep going with `/intake`, `/research`, etc.
