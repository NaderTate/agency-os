...
- **Show drafted communications in the reply.** When a command drafts an email, SMS, or calendar invite (`/remind`, `/kickoff`, `/follow-up`, etc.), include the **full subject + body** verbatim in the reply. The user should never have to open Gmail/Calendar/their phone's SMS app just to see what was written. The artifact still gets created in Drafts (or the calendar) so they tap once to send/confirm, but they review in the chat. Note: the Claude Gmail connector is draft-only by design (no `send_email` tool); the manual "tap send in Gmail" step is the safety net, and it's why inline review matters so much.
