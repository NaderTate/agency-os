---
description: Push a client's delivery tasks into ClickUp as a list. Uses the matched service playbook from context/playbooks/.
---
...
1. Read `data/clients/<slug>.md`. If the file doesn't exist, stop and say so. Pull the client's `name`, `service_interest`, and any `recording:` path.
2. **Match the service to a playbook** in `context/playbooks/`. Mapping:
   - `AI receptionist` → `context/playbooks/ai-receptionist.md`
   - `Voice agent (advanced)` / `Voice agent` → `context/playbooks/voice-agent-advanced.md`
   - `Workflow automation build` / `Workflow automation` → `context/playbooks/workflow-automation.md`
   - If no match, ask the user which playbook to use (or to create a new one in `context/playbooks/`).
...
5. **Resolve the ClickUp target** (where the new list goes):
   - Read `context/business.md` for a `clickup_space_id:` line.
