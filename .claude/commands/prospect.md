...
1. Read `context/business.md` (ICP, verticals you target, who you don't serve) and `context/services.md` (so prospects can be mapped to the right offer + price).
2. Read existing `data/clients/*.md` to build a set of already-known business names + slugs (you'll skip these so duplicates don't enter the pipeline).
...
4. **For each candidate found, score it** (1-10) against ICP:
   - Vertical match (must be in `context/business.md` list).
...
- This command writes to `data/clients/` (via `/intake`). Be conservative on auto-intake: better to surface 8 candidates and auto-intake 2 strong ones than to flood the pipeline with weak leads.
