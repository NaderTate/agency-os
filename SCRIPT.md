...
**[SHOW]** On-screen label: `Claude Code`. Then a title card listing the commands:
> `intake` · `research` · `proposal` · `kickoff` · `status`

**[SAY]**
> "Capture a lead, research the business, send a priced proposal, book the call, see the whole pipeline. That's an agency. And the thing actually doing the work isn't code I wrote, it's Claude reading a few markdown files."

**[SAY]** (the talk-to-it point, say it early)
> "And before anyone asks: no, you don't memorize commands. These are just shortcuts. You can literally talk to it. I'll show you. Let me open it up."
...
**[SAY]**
> "A lead just came in from one of my videos. A dental practice. Watch me put it in the system. And to prove the point about not memorizing commands, I'm not even going to type the slash command, I'll just tell it what happened."

**[DEMO]** Type it as plain English (this demonstrates natural-language routing):
```
new lead: Bright Now Dental in Austin, wants an AI receptionist, found me on YouTube
```
(The `/intake "<blurb>"` slash command does the exact same thing if you prefer the shortcut.)
...
## 8.5 ADOPT IT WITHOUT STARTING OVER, import (the skeptic-killer)

> This is the beat that converts the viewer who's thinking "cool, but I already run an agency on a real CRM." Don't skip it.

**[SAY]**
> "Now you're probably thinking: that's nice if you're starting fresh, but I already have an agency with clients in a CRM. Good. You don't start over. Export your clients to a CSV from whatever you use, GoHighLevel, HubSpot, a spreadsheet, and watch this."

**[DEMO]** Show the `clients.csv` file briefly (a few rows), then run:
```
import my clients from clients.csv
```
(Slash form: `/import clients.csv`.)

**[SHOW]** Claude creates a client file per row, mapping their stage names to its own. Then run `/status` (or say "show me the pipeline now") and the board fills with the imported book, pipeline value jumping.

**[SAY]**
> "One command. My whole client list is in, it mapped their stages, and now Claude runs my real book of business. Your CRM becomes the thing you exported from, not a tool you're stuck paying for. And if your CRM connects to Claude directly, you don't even need the CSV, you just tell it to pull them."
